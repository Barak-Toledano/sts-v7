...
class TaskManager:
    ...
    def create_task(self, coro: Coroutine, name: Optional[str] = None) -> asyncio.Task:
        ...
        # Track the task
        self.tasks.add(task)
        logger.debug(f"Task {task.get_name() if hasattr(task, 'get_name') else id(task)} created")
    ...
    def _task_done_callback(self, task: asyncio.Task) -> None:
        ...
        if not task.cancelled():
            ...
            if exception:
                task_name = task.get_name() if hasattr(task, 'get_name') else id(task)
    ...
    async def cancel_all(self, wait: bool = True, timeout: Optional[float] = 5.0) -> None:
        ...
        for task in list(self.tasks):
            if not task.done():
                logger.debug(f"Cancelling task {task.get_name() if hasattr(task, 'get_name') else id(task)}")
                task.cancel()
        ...
        if wait and self.tasks:
            ...
            try:
                ...
                if pending:
                    pending_tasks = [t.get_name() if hasattr(t, 'get_name') else id(t) for t in pending]
                    logger.warning(f"Some tasks didn't complete within timeout: {pending_tasks}")
