...
class AudioManager:
    ...
    async def play_audio(self, audio_data: bytes) -> bool:
        ...
        """
        try:
            await self.audio_service.play_audio(audio_data)
    ...
    async def pause_playback(self) -> bool:
        ...
        """
        try:
            await self.audio_service.pause_playback()
            return True
        except Exception as e:
            error = AppError(
    ...
    async def resume_playback(self) -> bool:
        ...
        """
        try:
            await self.audio_service.resume_playback()
            self.is_playing = True
            return True
        except Exception as e:
            error = AppError(
    ...
    def _handle_speech_finished(self, event: UserSpeechEvent) -> None:
        ...
        """
        logger.debug(f"Speech finished (duration: {event.duration:.2f}s)")
